/*
  ...
  Original copy by Hernando Barragan
  Modified by Mishel George 2015-2016
...
public abstract class Uploader implements MessageConsumer {
  ...
  protected boolean executeUploadCommand(String command[]) throws Exception {
    ...
    if (command == null || command.length == 0)
      return true;
	String [] hello= {"bash", "-c", " echo \"RST!\" > /dev/ttyS5"};
    try
        {
	        Process proc = ProcessUtils.exec(hello);
	        proc.waitFor();
        }
        catch (Exception e)
        {
    ...
    notFoundError = false;
    int result = -1;
	String[] actualCommand = new String[command.length + 1];
	actualCommand[0] = command[0];
	actualCommand[1] = "-V";
	for (int i = 2; i < command.length + 1; i++)
	{
		actualCommand[i] = command[i-1];
	}
	try {
      if (verbose) {
      ...
      Process process = ProcessUtils.exec(actualCommand);

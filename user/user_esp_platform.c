...
{
   spi_flash_read((ESP_PARAM_START_SEC+2) * SPI_FLASH_SEC_SIZE, (uint32 *)param, sizeof(esp_platform_saved_param));
}
...
{
   spi_flash_write((ESP_PARAM_START_SEC+2) * SPI_FLASH_SEC_SIZE, (uint32 *)param, sizeof(esp_platform_saved_param));
}
